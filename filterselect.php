...
$column = array('id', 'dateleased','name', 'address','phoneno','email','equipment','equipmentno','modelno','period','duration','leasedate','enddate','rent','sec','carriage','bookingadvance','balancedue','paymentrecedupto','lastservicedate','datepicked','gstin','flag','tobepicked','comments');
...
{
   // $row['leasedate'] = date('Y-m-d', strtotime($row['leasedate']));
   // $dt = new DateTime($row['leasedate']);
    //   $newdate =  $dt->format('d-m-Y');
    $dateleased=date("d-m-Y", strtotime($row['dateleased']));
    $enddate=date("d-m-Y", strtotime($row['enddate']));
    $leasedate=date("d-m-Y", strtotime($row['leasedate']));
    $paymentreceivedupto=date("d-m-Y", strtotime($row['paymentrecedupto']));
    $lastservicedate=date("d-m-Y", strtotime($row['lastservicedate']));
    $tobepicked=date("d-m-Y", strtotime($row['tobepicked']));
    $datepicked=date("d-m-Y", strtotime($row['datepicked']));
 $sub_array = array();
// $l=date("d-m-Y", strtotime($row['leasedate']));
 $sub_array[] = $row['id'];
  $sub_array[] = $dateleased;
 $sub_array[] = $row['name'];
 ...
 $sub_array[] =$row['period']." ".$row['duration'];
    $sub_array[] =$leasedate;
  $sub_array[] =$enddate;
 $sub_array[] = $row['rent'];
  $sub_array[] = $row['sec'];
   ...
   $sub_array[] = $row['balancedue'];
      $sub_array[] = $paymentreceivedupto;
       $sub_array[] = $lastservicedate;
        $sub_array[] = '<div contenteditable data-type="date" class="update" data-id="'.$row["id"].'" data-column="datepicked">' . $datepicked . '</div>';
        $sub_array[] = $row['gstin'];
        $sub_array[] = $row['flag'];
        $sub_array[]='<div contenteditable class="update" data-id="'.$row["id"].'" data-column="tobepicked">' . $tobepicked. '</div>';
